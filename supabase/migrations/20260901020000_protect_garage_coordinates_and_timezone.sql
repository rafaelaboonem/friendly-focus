...
alter table public.garage_private_details
  ...
  drop constraint if exists garage_private_details_exact_latitude_range,
  drop constraint if exists garage_private_details_exact_longitude_range,
...
create or replace function public.validate_garage_timezone()
returns trigger
language plpgsql
security invoker
...
begin
  if new.timezone is not null and not exists (
    ...
    raise exception 'Timezone IANA inválido';
  end if;

  if new.status = 'active' and new.timezone is null then
    raise exception 'Garagem ativa precisa de timezone IANA';
  end if;
...
drop trigger if exists garages_validate_timezone on public.garages;
create trigger garages_validate_timezone
before insert or update of timezone, status on public.garages
for each row execute procedure public.validate_garage_timezone();
...
begin
  ...
  if not found or v_garage.timezone is null then
    raise exception 'Garagem indisponível para reservas';
  end if;

  v_start_local := p_starts_at at time zone v_garage.timezone;
  v_end_local := p_ends_at at time zone v_garage.timezone;

  if v_start_local::date <> v_end_local::date then
    raise exception 'Reservas que atravessam dias locais ainda não são suportadas';
  end if;
